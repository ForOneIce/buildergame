...
export class SupportError extends Error { code: string; stage?: string; constructor(code: string, message?: string); }
...
export interface DonationState { phase: DonationPhase; intent: DonationIntent | null; estimate: DonationEstimate | null; hash: string | null; error: string | null; errorStage: string | null; critical: boolean }
...
export interface TransferGuard { acquire(): Promise<void>; save(checkpoint: DonationCheckpoint | null): void; read(): (DonationCheckpoint & { eventId: string; attemptId: string; legacyVersion?: 1 }) | null; migrate(saved: DonationCheckpoint): Promise<void>; release(): void }
export function createBrowserTransferGuard(options: { eventId: string; storage?: Storage; locks?: LockManager; randomId?: () => string; assertNoLegacy?: () => void }): TransferGuard;
export function atSupportStage<T>(stage: string, task: () => Promise<T>): Promise<T>;
export function sameRecoveryIntent(left: unknown, right: unknown): boolean;
export function assertNoLegacyRecovery(storage: Pick<Storage, 'getItem'>, key: string): void;
