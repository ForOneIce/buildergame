...
export function cleanEvent(event) {
  ...
  return { ...only(event, ['schemaVersion', 'id', 'name', 'subtitle', 'url', 'sampleData', 'collectionType', 'landscape', 'mode', 'refreshSeconds', 'customScores']), rule: cleanRule(event.rule), projects: event.projects.map(p => ({ ...only(p, ['id', 'name', 'repository', 'description', 'homepage', 'color']), plot: only(p.plot, ['x', 'z']), builder: only(p.builder, ['name', 'bio', 'url', 'avatar', 'followers']) })) };
}
