...
const names = ['Open Orchard', 'Little Atlas', 'Cloud Notes', 'Garden Kit', 'Kindred', 'Moon Studio', 'Paper Trail', 'Seedling'];
export function sampleTown() {
  ...
  const event = { schemaVersion: 1, id: 'sample-town', name: 'The Builder Islands', subtitle: 'Small steps. A world of possibilities.', url: '', sampleData: true, collectionType: 'hackathon', mode: 'static', refreshSeconds: 300, rule, projects };
  const counts = [[0, 4, 22, 42, 12, 70, 0, 140], [0, 16, 35, 65, 50, 100, 8, 200], [0, 22, 45, 120, 75, 150, 20, 230]];
  const snapshots = counts.map((commits, j) => {
    const capturedAt = `2026-09-${String(5 + j * 2).padStart(2, '0')}T12:00:00.000Z`;
    return { id: `sample-day-${j}`, label: ['First foundations', 'Finding our rhythm', 'A neighborhood in bloom'][j], capturedAt, projects: projects.map((p, i) => makeRecord(p, { commits: commits[i], stars: commits[i] ? i * 4 + j * 5 : 0, forks: commits[i] ? Math.floor(i * (j + 1) * 1.6) : 0 }, rule, capturedAt)) };
  });
