...
function header() {
  if(screen==='town')return gameHeader(t,session,lang);
...
function render() {
  document.body.classList.toggle('town-view',screen==='town');
...
function bindSetup() {
  ...
  picker.querySelector('select')!.onchange=e=>{landscape=(e.target as HTMLSelectElement).value as Landscape;if(imported&&!usePrevious)imported.landscape=landscape;};
...
function details(id: string) {
  ...
  const body='<span class="stage-pill">'+stage(r.stage)+'</span><p class="helper">'+(r.observedAt?t('Observed','采集于')+' '+date(r.observedAt)+' UTC':t('Data unavailable','暂无数据'))+(r.status==='stale'?t(' · Last known state',' · 上次记录'): '')+'</p>';
  const stats=(['stars','commits','forks'] as const).map((k,i)=>'<span>'+['★','⌘','⑂'][i]+' '+(r.metrics?.[k]?.toLocaleString()??'—')+' '+k+'</span>').join('');
  const links=noSampleLinks?'<p class="helper">'+t('Fictional sample project. Explore a real town to visit repositories.','虚构示例项目；真实小镇可访问对应仓库。')+'</p>':link(projectDestination(p),t('Visit project','访问项目'),'button primary')+link(p.repository,t('Star on GitHub','前往 GitHub 星标'))+link(p.builder.url,t('Follow builder','关注开发者'));
  $('#detail-body').innerHTML=projectCard(p,body,links,stats,t);
  markVisited(id);
  scene?.capturePreview(data=>{if(selected!==id)return;const img=document.querySelector<HTMLImageElement>('#project-preview-image');if(img){img.src=data;img.hidden=false;document.querySelector<HTMLElement>('#project-preview-placeholder')!.hidden=true;}});
...
function bindTown() {
  ...
  $('#show-exploration').onclick=()=>{$('#next-project').focus();if(!matchMedia('(prefers-reduced-motion: reduce)').matches)document.querySelector<HTMLElement>('.exploration-panel')!.animate([{opacity:.4},{opacity:1}],{duration:450});};
  ...
  showSnapshot(snapshotIndex);refreshProgress();void syncProgress();
}
