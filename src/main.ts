...
function exportTown() { const slug=bundle.event.deployment?.slug;download(publicBundle(bundle),slug?`${slug}.json`:'town.json');notice(hasSupport(bundle.event)?t('Backup downloaded with receiving addresses. Keep it local for testing; import it on the deployed site without publishing the file.','备份已下载，内含收款地址。测试时请保留在本地，可在已部署站点导入，无须公开文件。'):slug?t(`Backup downloaded. Add ${slug}.json to public/data/towns/ in your GitHub deployment repository.`,`备份已下载。将 ${slug}.json 添加到 GitHub 部署仓库的 public/data/towns/。`):t('Backup downloaded. Add town.json to public/data/ in your deployment repository, then rebuild.','备份已下载。将 town.json 放入部署仓库的 public/data/，然后重新构建。')); }
  ...
  return `<main class="setup-page planning-desk" aria-label="${t('Town planning sheet','小镇规划图纸')}">
    <div class="planning-sheet" ${planTurn?`data-page-turn="${planTurn}"`:''}><div class="setup-grid">
      ...
      <section class="setup-card" aria-label="${t('Town configuration','小镇配置')}">
        ...
        <p class="helper publish-hint">${canPublishHere()?t('Saved at its own address. Your other towns stay available.','保存到独立地址，其他小镇仍可访问。'):t('Create in your browser and keep a local backup. Review its contents before publishing a shared town address.','在浏览器中创建并保留本地备份；公开小镇地址前，请先检查备份内容。')}</p>
...
function success() {
  ...
  const walletConfigured=hasSupport(bundle.event);
  return '<main class="success-page paper-panel"><span class="success-icon">'+objectArt('map')+'</span><p class="eyebrow">'+t('A NEW CHAPTER BEGINS','新的篇章开始了')+'</p><h1>'+t('Your town is ready.','你的小镇建好了。')+'</h1><p>'+escape(bundle.event.name)+' · '+bundle.event.projects.length+' '+t('projects','个项目')+' · '+measured+' '+t('recorded snapshots','张记录快照')+'</p><p class="success-status">'+(published?t('Saved on this site. Anyone with the town link can explore without signing in.','已保存在此站点。任何人都可以通过小镇链接免登录游览。'):walletConfigured?t('Local preview. Keep the backup on your device to continue testing.','本地预览。请将备份保留在设备上，以便继续测试。'):t('Local preview. Download the backup to keep it and publish it from your GitHub repository.','本地预览。请下载备份留存，也可以从 GitHub 仓库部署发布。'))+'</p>'+
