...
Acceptance: planner-only loop, no overlapping tracks, stop on every exit, proper mute/gesture behavior, and no track on other screens. [The submission video](../prompts/0051-demo-video-audio-versions.md) keeps background music off while retaining interface sound, with a separately exported no-audio alternative. Added narration is not required by the adopted initial-demo plan; an optional live script is prepared separately.
