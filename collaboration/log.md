...
## Final demonstration artifacts

- 2026-09-13 (UTC): the final video contains human English narration, original interface effects and no music. Video and audio both measure 217.68 seconds; the 1280 × 720, 25 fps video contains 5,442 frames. Full decoding and visual privacy checks passed.
- The actual viewport shows sample exploration, a town created from anonymous public GitHub data and a personally approved 0.001 Sepolia test-ETH transfer with a matching successful application receipt. Codex assisted capture, editing and privacy masking. Final presentation and image provenance are in [judge materials](../hackathon/README.md) and [media](../hackathon/media/README.md).
- [Artifact verification](../docs/verification.md#updated-recorded-demonstration--2026-09-13-utc).

## Successful hosted Sepolia transfer

- 2026-09-13 (UTC), phase: deployed transaction verification. After the earlier low-balance check, the human supplied test funds and personally confirmed the final Privy transaction. The app reported that 0.001 Sepolia test ETH reached the configured recipient. Codex activated Back to town, observed the support card close and focus return to the header wallet control, then opened Projects to show the captured-repository directory. This verified release of the interaction lock and basic navigation after success.
- Codex independently opened the app's explorer link and checked Sepolia Etherscan Success, the 0.001 ETH value and matching sender/recipient. Nine confirmations were visible with an unfinalized state; no finality claim is made. This timber-frame town had no garden mailbox, so the app success card and explorer establish the transfer without an observed coin-animation claim. Codex updated `README.md`, `hackathon/summitinfo.md`, `docs/verification.md`, this log and `collaboration/AI_USAGE.md`, retaining the earlier activation and low-balance evidence as history.
- Codex then reopened the settled card for a separate review, obtained a fresh default-amount quote with a visible fee and selected Not now in Privy's final preview. The app reported cancellation before submission with no transfer or network fee. The Privy modal hid, the support card closed and Projects expanded normally. No second transaction was approved; live cancellation and interaction unlock were verified.
- The latest automated counts remain 82 Node tests, including 33 transaction fixtures, and 21 compact-wallet scenarios. No code or new tests changed. Interrupted session/reload recovery remains unverified. [Successful transfer and cancellation observations](../docs/verification.md#successful-hosted-sepolia-transfer-observed-on-2026-09-13-utc).
...
- At this earlier checkpoint, funded review, fee estimation, transfer approval, an actual Sepolia transfer/receipt, live recovery and human acceptance of the complete flow remained pending. No transaction was sent during the low-balance check. [Earlier observation and limits](../docs/verification.md#hosted-wallet-activation-and-login-observed-on-2026-09-13-utc).
