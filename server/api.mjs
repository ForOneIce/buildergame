...
export function createApi(env = process.env, options = {}) {
  ...
  const playerConfigured = Boolean(env.GITHUB_CLIENT_ID && env.GITHUB_CLIENT_SECRET);
  const progress = progressStore(resolve(env.PLAYER_DATA_DIR || 'private/players'));
  let busy = false;
  async function load() {
  ...
  return async function api(req, res) {
    ...
    try {
      ...
      if (url.pathname === '/api/session' && req.method === 'GET') return json({ configured, playerConfigured, authenticated, isDeployer, login: session?.login || null, avatar: session?.avatar || null });
      if (url.pathname === '/api/progress' && ['GET','POST'].includes(req.method)) {
        if (!authenticated) return json({error:'Sign in to synchronize exploration'},401);
        const input=req.method==='POST'?await body():{townId:url.searchParams.get('town')};
        const town=await load();if(!town || input.townId!==town.event.id)return json({error:'Progress sync is available for the deployed town'},404);
        const known=new Set(town.event.projects.map(p=>p.id));
        if(req.method==='POST' && !known.has(input.projectId))return json({error:'Unknown project'},400);
        const result=req.method==='POST'?await progress.visit(session.login,town.event.id,input.projectId):await progress.read(session.login,town.event.id);
        return json({...result,visited:result.visited.filter(id=>known.has(id))});
      }
      ...
      if (url.pathname === '/api/auth/login' && req.method === 'GET') {
        ...
        const state = tokenId(); states.set(state, { expires: now + 600000, player });
      ...
      if (url.pathname === '/api/auth/callback' && req.method === 'GET') {
        ...
        const player=states.get(state).player;states.delete(state);
        ...
        if (!player && user.login.toLowerCase() !== env.GITHUB_ALLOWED_LOGIN.toLowerCase()) return json({ error: 'This GitHub account is not the configured town deployer' }, 403);
        if(cookies.bg_session)sessions.delete(cookies.bg_session);
        const id = tokenId(); sessions.set(id, { token: grant.access_token, login: user.login, avatar: user.avatar_url || null, expires: now + 8 * 3600000 });
        return redirect(player?'/?town=1':'/?setup=personal', [cookie('bg_session', id, secure, 28800), cookie('bg_state', '', secure, 0)]);
      }
      ...
      if (url.pathname === '/api/capture' && req.method === 'POST') {
        ...
        if (input.publish && !isDeployer) return json({ error: 'Sign in as the configured deployer to publish' }, authenticated?403:401);
