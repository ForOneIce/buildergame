...
1. Prepare a town backup in a local or already deployed buildergame instance. Choose **Connect GitHub**, supply a personal access token, select public repositories and capture a snapshot. Review the export before deciding to publish it. Wallet-enabled backups retain receiving addresses: keep test-recipient files local and use the import workflow below. Do not copy private server storage envelopes, OAuth state or credentials into `public/`.
...
For an optional-wallet test, deploy the source with `VITE_PRIVY_APP_ID` set in Vercel's relevant **build environment** and its exact origin allowed in Privy; rebuild after changing that value. Keep the recipient-containing JSON in an ignored local folder. On the deployed app, use **Create town → Not ready yet? → Load a plan** to import the complete bundle. This restores a local preview in that browser without publishing the JSON or committing receiving addresses. The App ID is public in the built client; receiving addresses are observable in runtime wallet data and on-chain activity. This workflow keeps them out of GitHub, rather than making them secret on the blockchain. [Complete import steps](deployment.md#test-a-local-backup-on-a-deployed-site).
