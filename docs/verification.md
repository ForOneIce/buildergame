...
The implemented recovery path stores only public pending transaction details (intent, sender, state and known hash) in `sessionStorage`, keyed by town. It does not store private keys, signatures, login tokens or this metadata in town exports. Returning to a pending flow permits receipt checking, not an automatic resend. Storage availability and real reload/authentication recovery still require browser/live-flow verification; the controller fixtures alone do not establish that complete hosted path.
