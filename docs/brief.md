...
Real Web3 work is paused. The implemented [sample-town mailbox Easter egg](../specs/0013-sample-mailbox-coins.md) lets visitors hover a stage-five mailbox for a coin cursor, play a virtual deposit animation and open a future wallet/sponsorship explanation below landscape tours. It is a visual simulation with a temporary view-local coin count and no payment, provider connection, saved balance or growth change. Scoped mailbox browser checks and the final production build passed; human visual review remains pending. The earlier [Web3 discussion](ideation/0043-web3-support-options.md) remains research with no selected wallet, chain or partner integration.
